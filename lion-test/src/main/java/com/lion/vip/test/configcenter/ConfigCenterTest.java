...
package com.lion.vip.test.configcenter;
...
public class ConfigCenterTest {
    ...
    public void testKey() {
        ...
        System.out.println(CC.lion.push.flow_control.global.max);
    }
    ...
    public static void print(String s, ConfigValue configValue, Map<String, String> map) {
        if (s.startsWith("lion") && !s.endsWith("\"")) {
            String[] keys = s.split("\\.");
    ...
    public void testLoad2() {
        Map<String, String> map = new HashMap<>();
        System.out.println("public interface lion {");
        System.out.printf("  Config cfg = ConfigManager.I.lion().toConfig();%n%n");
        CC.lion.cfg.root().forEach((s, configValue) -> print2(s, configValue, "lion", 1));
        System.out.println("}");
    }
