...
public class RollingNumber {
    ...
    /* package for testing */ RollingNumber(Time time, int timeInMilliseconds, int numberOfBuckets) {
        this.time = time;
    ...
    /**
     * Increment the counter in the current bucket by one for the given {@link Event} type.
     * <p>
     * The {@link Event} must be a "counter" type <code>HystrixRollingNumberEvent.isCounter() == true</code>.
     *
     * @param type HystrixRollingNumberEvent defining which counter to increment
     */
    public void increment(Event type) {
        getCurrentBucket().getAdder(type).increment();
    }

    /**
     * Add to the counter in the current bucket for the given {@link Event} type.
     * <p>
     * The {@link Event} must be a "counter" type <code>HystrixRollingNumberEvent.isCounter() == true</code>.
     *
     * @param type  HystrixRollingNumberEvent defining which counter to add to
     * @param value long value to be added to the current bucket
     */
    public void add(Event type, long value) {
    ...
    /**
     * Get the cumulative sum of all buckets ever since the JVM started without rolling for the given {@link Event} type.
     * <p>
     ...
     * @return cumulative sum of all increments and adds for the given {@link Event} counter type
     */
    public long getCumulativeSum(Event type) {
    ...
    /**
     * Get the sum of all buckets in the rolling counter for the given {@link Event} type.
     * <p>
     * The {@link Event} must be a "counter" type <code>HystrixRollingNumberEvent.isCounter() == true</code>.
     *
     * @param type HystrixRollingNumberEvent defining which counter to retrieve values from
     * @return value from the given {@link Event} counter type
     */
    public long getRollingSum(Event type) {
    ...
    /**
     * Get the value of the latest (current) bucket in the rolling counter for the given {@link Event} type.
     * <p>
     * The {@link Event} must be a "counter" type <code>HystrixRollingNumberEvent.isCounter() == true</code>.
     *
     * @param type HystrixRollingNumberEvent defining which counter to retrieve value from
     * @return value from latest bucket for given {@link Event} counter type
     */
    public long getValueOfLatestBucket(Event type) {
        Bucket lastBucket = getCurrentBucket();
        if (lastBucket == null)
            return 0;
    ...
    /**
     * Get an array of values for all buckets in the rolling counter for the given {@link Event} type.
     * <p>
     ...
     *
     * @param type HystrixRollingNumberEvent defining which counter to retrieve values from
     * @return array of values from each of the rolling buckets for given {@link Event} counter type
     */
    public long[] getValues(Event type) {
        Bucket lastBucket = getCurrentBucket();
        if (lastBucket == null)
            return new long[0];
    ...
    /**
     * Get the max value of values in all buckets for the given {@link Event} type.
     * <p>
     * The {@link Event} must be a "max updater" type <code>HystrixRollingNumberEvent.isMaxUpdater() == true</code>.
     *
     * @param type HystrixRollingNumberEvent defining which "max updater" to retrieve values from
     * @return max value for given {@link Event} type during rolling window
     */
    public long getRollingMaxValue(Event type) {
    ...
    /* package */static class Bucket {
        ...
        Bucket(long startTime) {
            ...
            adderForCounterType = new LongAdder[Event.values().length];
